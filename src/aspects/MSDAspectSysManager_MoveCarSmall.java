//  The file MSDAspectSysManager_MoveCarSmall.aj was automatically generated using S2A 2.0.0.qualifier.
...
public class MSDAspectSysManager_MoveCarSmall extends MSDAspect implements MSDSubscribe
{
	...
	static final int int_arg_int_exact_int_distance1 = 0;
	static final int int_arg_int_exact_int_distance0 = 1;

	static final int int_var_symbolic_minimalDistance = 2;

	private final String niceName = "SysManager_MoveCarSmall";

	private static MSDAspectSysManager_MoveCarSmall instance = null;

	public static MSDAspectSysManager_MoveCarSmall getInstance(){
		if(instance==null){
			instance = new MSDAspectSysManager_MoveCarSmall();
		}
		return instance;
	}

	private MSDAspectSysManager_MoveCarSmall()
	{
		addAspectEvent(MSDMethods.Env_SysManager_runCycleSmall);
		PlaygoCoordinator.getInstance().subscribe(this, 
			"Env", "SysManager", "runCycleSmall");
		addAspectEvent(MSDMethods.Car_Car_moveCar);
		PlaygoCoordinator.getInstance().subscribe(this, 
			"Car", "Car", "moveCar");

		addMinimalEvent(MSDMethods.Env_SysManager_runCycleSmall);
		setHotCut(4,4,4,3);
		setHotCut(3,3,2,2);
	...
	protected boolean changeActiveMSDCutState(int MSDm, Object sourceObject, 
		...
		switch (MSDm)
		{
			case MSDMethods.Env_SysManager_runCycleSmall:
	...
	protected void doBindings(ActiveMSDAspect activeMSD)
	{
		...
		try{
			activeMSD.setPrivateVariable(int_arg_int_exact_int_distance1, (int)-20);
		}catch(Exception e){}
		try{
			activeMSD.setPrivateVariable(int_arg_int_exact_int_distance0, (int)20);
		}catch(Exception e){}
	}
	...
	private void after_Env_SysManager_runCycleSmall(Object _user ,Object _sysManager)
	{
		changeCutState(MSDMethods.Env_SysManager_runCycleSmall,
	...
	{
		if(srcClassName.equals("Env") && tgtClassName.equals("SysManager") && method.equals("runCycleSmall")){
			after_Env_SysManager_runCycleSmall(source, target);
			return;
		}
			...
			MSDMethodSet CV,MSDMethodSet HV,ActiveMSDAspect activeMSD)
	{
		...
		MSDMethod MSDm38 = new MSDMethod(User,sysManager,
			MSDMethods.Env_SysManager_runCycleSmall,null,"Env:SysManager:runCycleSmall",niceName,activeMSD.getGlobalId());// runCycleSmall() Monitored
		ArrayList<Object>  args39 = getArgsList(VAR_int_exact_int_distance0);
		MSDMethod MSDm39 = new MSDMethod(car,car,
			MSDMethods.Car_Car_moveCar,args39,"Car:Car:moveCar",niceName,activeMSD.getGlobalId());// moveCar() Execute
		ArrayList<Object>  args40 = getArgsList(VAR_int_exact_int_distance1);
		MSDMethod MSDm40 = new MSDMethod(car,car,
			MSDMethods.Car_Car_moveCar,args40,"Car:Car:moveCar",niceName,activeMSD.getGlobalId());// moveCar() Execute

		 if(activeMSD.isInCut(0,0,0,0))
		{
				ME.add(MSDm38);
				CV.add(MSDm39,MSDm40);
				return;
		}
		 if(activeMSD.isInCut(3,3,2,2))
		{
