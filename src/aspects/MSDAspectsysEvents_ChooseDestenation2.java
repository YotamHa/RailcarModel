//  The file MSDAspectsysEvents_ChooseDestenation2.aj was automatically generated using S2A 2.0.0.qualifier.
...
public class MSDAspectsysEvents_ChooseDestenation2 extends MSDAspect implements MSDSubscribe
{
	...
	private final String niceName = "sysEvents_ChooseDestenation2";

	private static MSDAspectsysEvents_ChooseDestenation2 instance = null;

	public static MSDAspectsysEvents_ChooseDestenation2 getInstance(){
		if(instance==null){
			instance = new MSDAspectsysEvents_ChooseDestenation2();
		}
		return instance;
	}

	private MSDAspectsysEvents_ChooseDestenation2()
	{
		addAspectEvent(MSDMethods.Env_SysManager_chooseTerminal);
		PlaygoCoordinator.getInstance().subscribe(this, 
			"Env", "SysManager", "chooseTerminal");
		...
		addMinimalEvent(MSDMethods.Env_SysManager_chooseTerminal);
		setHotCut(1,0,1);
		setHotCut(2,1,1);
		setLastCut(3,2,1);
		numberOfLifeLines = 3;
	...
	protected boolean changeActiveMSDCutState(int MSDm, Object sourceObject, 
		...
		switch (MSDm)
		{
			...
			case MSDMethods.Env_SysManager_chooseTerminal:
	...
	private void after_Env_SysManager_chooseTerminal(Object _user ,Object _sysManager)
	{
		changeCutState(MSDMethods.Env_SysManager_chooseTerminal,
	...
	{
		if(srcClassName.equals("Env") && tgtClassName.equals("SysManager") && method.equals("chooseTerminal")){
			after_Env_SysManager_chooseTerminal(source, target);
			return;
		}
			...
			MSDMethodSet CV,MSDMethodSet HV,ActiveMSDAspect activeMSD)
	{
		...
		MSDMethod MSDm179 = new MSDMethod(sysManager,car1,
		...
		MSDMethod MSDm180 = new MSDMethod(User,sysManager,
		...
		ArrayList<Object>  args181 = getArgsList(VAR_int_exact_int_arg00);
		MSDMethod MSDm181 = new MSDMethod(sysManager,car1,
			MSDMethods.SysManager_Car_setDestination,args181,"SysManager:Car:setDestination",niceName,activeMSD.getGlobalId());// setDestination() Execute

		 if(activeMSD.isInCut(0,0,0))
		{
				ME.add(MSDm180);
				CV.add(MSDm181,MSDm179);
				return;
		}
		 if(activeMSD.isInCut(1,0,1))
		{
				if(MSDm181.sourceInstance!=null && MSDm181.targetInstance!=null){
					EE.add(MSDm181);
				}
				HV.add(MSDm180,MSDm179);
				return;
		}
		 if(activeMSD.isInCut(2,1,1))
		{
				if(MSDm179.sourceInstance!=null && MSDm179.targetInstance!=null){
					EE.add(MSDm179);
				}
				HV.add(MSDm181,MSDm180);
				return;
		}
	}
