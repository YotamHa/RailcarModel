...
public class PlayableContainer extends PlayableFramework implements IPlayableComponent, IPlayableContainer 
{
	...
	public void objectEvent(String className, String objectName,
		...
		while (components.hasNext())
		{
				...
				(next.getPropertyOrMethodName()!= null) && next.propertyOrMethodName.equals(eventName))
			{
				...
				this.framework.objectEvent(this.className, this.name, containerMethodName, 
						argTypes, argValues);
			}
			index++;
		}
			...
			String propertyName, String type, String value) 
	{
		...
		while (components.hasNext())
		{
			...
			{
				containerMethodName = (String) containedComponentsMap.keySet().toArray()[index];
				this.framework.objectPropertyChanged(this.className, this.name, 
						containerMethodName, type, value);
			}
			index++;
		}
			...
			String propertyName, String type, String value) 
	{
		...
		while (components.hasNext())
		{
			...
			{
				containerMethodName = (String) containedComponentsMap.keySet().toArray()[index];
				this.framework.objectPropertyChanged(sourceClassName, sourceObjectName, 
						this.className, this.name, containerMethodName, type, value);
			}
			index++;
		}
